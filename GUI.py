...
# Ubicaciones de Variables (Donde estan lo que dice Gas y el %)
variableGas.place(x=70, y=30)
label.place(x=50, y=150)

# Crear el widget Canvas para el gráfico (Cuadrado del Grafico)
...
def actualizar_porcentaje():
    ...
    if porcentaje < 100:  # Limita el porcentaje hasta 100
        ...
        if porcentaje < 34:
            ...
            variableGas.config(fg="#E05E26")  # Mas de 33% y menos de 67% (Amarillo)
            label.config(fg="#E05E26") #(Amarillo)
        else:
            variableGas.config(fg="#FF0000")  # Mas de 66% (Rojo)
